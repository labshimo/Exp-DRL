...
class Dueling_CNN:
    def __init__(self,current_id,learning_rate=0.01, state_size=80, position_num=2, action_size=2, hidden_size=80):
        ...
        middlelayer = Dense(hidden_size, name='Dense1')(middlelayer)
        middlelayer = BatchNormalization(name='Batch2')(middlelayer)
        middlelayer = Activation('relu')(middlelayer)
        # dueling network
        y=Dense(self.action_size + 1, activation='linear', name='Dense2')(middlelayer)     # 0番目がV(s), 1以降がA(s,a), 平均値は引かないnaive型にする
