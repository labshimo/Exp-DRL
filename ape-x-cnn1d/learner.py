...
class Learner:
    def __init__(self,
        ...
        with tf.variable_scope("learner_target_parameters",  reuse=False):
    ...
    def run(self):
        ...
        #for text
        out = 'AVERAGE LOSS,AVG_MAX_Q,LEARN PER SECOND,NUM LEARN\n'
        with open(self.path + '/learner.csv','a') as f:
            f.write(out)
        while self.no_added_count < self.max_queue_no_added:
            ...
            target_q_values_batch = self.target_q_values.eval(feed_dict={self.st: np.float32(next_state_batch)}, session=self.sess)
            # DDQN
            actions = np.argmax(self.q_values.eval(feed_dict={self.s: np.float32(next_state_batch)}, session=self.sess), axis=1)
            ...
            if self.t % self.print_interval == 0:
                ...
                out = '{0:.5F},{1:2.4F},{2:.1F},{3:5d}\n'.format(self.total_loss/self.print_interval, self.total_q_max/(self.print_interval*self.batch_size), self.print_interval/self.total_time, self.t)
                print(text_l)
                with open(self.path + '/learner.csv','a') as f:
                    f.write(out)
                self.total_loss  = 0
